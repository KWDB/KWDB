...
**剩余缺口（已清零，见 §2.6 / 阶段4）**：投影 CASE/COALESCE、窗口 ROWS 偏移 frame、过滤 IN 的 decimal 集合三项，以及**日期时间函数**（`extract`/`date_trunc`/`now`）均已补齐。阶段 C 表达式全覆盖的 Arrow 化清单至此基本落地（`now()` 经 2026-08-10 新增的 `evalArrowDatetimeFunc` `now` 分支落地：planner `arrowDatetimeFuncName` 归一化 `now`/`current_timestamp`/`transaction_timestamp`→`"now"`，`canArrowDatetime` 允许 0 参数，`addArrowRendering` 产出 `{Kind:"datetime",Func:"now",TZ:true}`，executor 用 `evalCtx.GetStmtTimestamp()` 写等长常量 TIMESTAMPTZ 数组，单测 `arrow_projection_now_test.go` 验证；`go build ./pkg/sql` 通过）。`age(ts1,ts2)` 双输入时间函数仍走通用回退——因其结果类型为 INTERVAL，而 vendored arrow 无 interval 减法 kernel、且 interval↔Arrow Duration 双向类型门控需改动 `arrow_adapter.go`，暂未纳入（功能正确，属可选优化，见 §6.5 末尾）。
