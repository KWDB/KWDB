			...
			case types.DecimalFamily:
				...
				case sqlbase.DataType_DOUBLE:
					...
					for j := 0; uint32(j) < rev.DataCount; j++ {
						...
						(rev.Data[i].Decimal()[j]).SetString(strconv.FormatFloat(value, 'f', -1, 64))
					}
				case sqlbase.DataType_DECIMAL:
					...
					for j := 0; uint32(j) < rev.DataCount; j++ {
						...
						if val[0] != 1 {
							...
							(rev.Data[i].Decimal()[j]).SetString(strconv.FormatFloat(data, 'f', -1, 64))
						}
					}
				}
			...
			case types.IntervalFamily:
				...
				case sqlbase.DataType_TIMESTAMP_MICRO, sqlbase.DataType_TIMESTAMPTZ_MICRO:
					for j := 0; uint32(j) < rev.DataCount; j++ {
						...
						var extraDays int64 = value / (Day / 1000)
						var days int64 = extraDays
				...
				case sqlbase.DataType_TIMESTAMP_NANO, sqlbase.DataType_TIMESTAMPTZ_NANO:
					for j := 0; uint32(j) < rev.DataCount; j++ {
						...
						var extraDays int64 = value / (Day / 1)
						var days int64 = extraDays
				...
				default:
					for j := 0; uint32(j) < rev.DataCount; j++ {
						...
						var extraDays int64 = value / (Day / 1000000)
						var days int64 = extraDays
