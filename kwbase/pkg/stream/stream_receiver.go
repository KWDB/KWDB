...
type StreamReceiver struct { // nolint
...
func init() {
	sql.NewStreamResultWriter = NewStreamResultWriter
}
...
func (srw *StreamReceiver) Close() {
	// Cancel the context to stop the async flush/checkpoint goroutine.
	if srw.cancel != nil {
		srw.cancel()
	}
