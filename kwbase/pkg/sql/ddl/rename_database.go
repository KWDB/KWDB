...
var _ sql.PlanNode = &renameDatabaseNode{}
...
func RenameDatabase(
	ctx context.Context, p *GenericPlanner, n *tree.RenameDatabase,
) (sql.PlanNode, error) {
	if n.Name == "" || n.NewName == "" {
...
func (n *renameDatabaseNode) StartExec(params RunParams) error {
	...
	for _, schema := range schemas {
		...
		for i := range tbNames {
			...
			for _, dependedOn := range tbDesc.DependedOnBy {
				found, procName, err := sql.GetProcedureNameByID(ctx, params.ExecCfg().InternalExecutor, params.GetTxn(), dependedOn.ID)
				if err != nil {
					return err
				}
				if found {
					err = params.GetPlanner().TryPurgeProcedureCache(ctx, procName, uint32(dependedOn.ID))
					if err != nil {
						return err
					}
					continue
				...
				if dbDesc.ID != dependentDesc.ParentID || tbDesc.GetParentSchemaID() != dependentDesc.GetParentSchemaID() {
					var err error
					dependentDescQualifiedString, err = sql.GetQualifiedTableName(ctx, p.Txn(), dependentDesc)
					if err != nil {
						log.Warningf(
							ctx,
	...
	if err := sql.RenameDatabase(ctx, p, dbDesc, n.newName); err != nil {
		return err
	}
...
func isAllowedDependentDescInRenameDatabase(
	...
	tbDesc *TableDescriptor,
	dependentDesc *TableDescriptor,
	dbName string,
) (bool, string, error) {
	...
	for _, column := range dependentDesc.Columns {
		...
		seqNames, err := sql.GetUsedSequenceNames(typedExpr)
		if err != nil {
			return false, "", err
		}
