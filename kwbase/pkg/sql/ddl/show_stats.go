...
// dummy
type showTableStatsNode struct {
	_ string
}
...
func ShowTableStats(
	ctx context.Context, p *GenericPlanner, n *tree.ShowTableStats,
) (sql.PlanNode, error) {
	// timeseries check
	...
	desc, err := p.ResolveUncachedTableDescriptorEx(ctx, n.Table, true /*required*/, sql.ResolveRequireTableDesc)
	if err != nil {
		return nil, err
	}
	...
	return sql.NewDelayedNode(
			n.String(),
			columns,
			func(ctx context.Context, p *GenericPlanner) (_ sql.PlanNode, err error) {
				...
				if n.UsingJSON {
					...
					if _, err := v.AddRowToValueNode(ctx, tree.Datums{tree.NewDJSON(jsonResult)}); err != nil {
						v.Close(ctx)
						return nil, err
					}
					return v, nil
				...
				for _, r := range rows {
					...
					if _, err := v.AddRowToValueNode(ctx, res); err != nil {
						v.Close(ctx)
						return nil, err
					}
				}
