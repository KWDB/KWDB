...
type dropCascadeState struct {
	...
	objectNamesToDelete     []sql.ObjectName
	td                      []sql.TableToDelete
	allTableObjectsToDelete []*MutableTableDescriptor
...
func (d *dropCascadeState) collectObjectsInSchema(
	ctx context.Context, p *GenericPlanner, db *DatabaseDescriptor, schema *sqlbase.ResolvedSchema,
) error {
	names, err := sql.GetObjectNames(ctx, p.Txn(), p, db, schema.Name, true /* explicitPrefix */)
	if err != nil {
		return err
	}
	for i := range names {
...
func (d *dropCascadeState) resolveCollectedObjects(
	ctx context.Context, p *GenericPlanner, db *DatabaseDescriptor,
) error {
	d.td = make([]sql.TableToDelete, 0, len(d.objectNamesToDelete))
	...
	for i := range d.objectNamesToDelete {
		...
		if found {
			tbDesc, ok := desc.(*MutableTableDescriptor)
			if !ok {
			...
			if err := sql.PrepareDropWithTableDesc(ctx, p, tbDesc); err != nil {
				return err
			}
			...
			d.td = append(d.td, sql.TableToDelete{&objName, tbDesc})
		}
	}

	allObjectsToDelete, implicitDeleteMap, err := accumulateAllObjectsToDelete(ctx, p, d.td)
	if err != nil {
		return err
	}
...
func (d *dropCascadeState) dropAllCollectedObjects(ctx context.Context, p *GenericPlanner) error {
	...
	for _, toDel := range d.td {
		...
		if desc.IsView() {
			...
			cascadedObjects, err = sql.DropTableImpl(ctx, p, desc, false /* droppingParent */, "", tree.DropCascade)
		}
		if err != nil {
			return err
		}
