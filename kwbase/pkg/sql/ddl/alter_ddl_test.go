...
func TestAlterScheduleNodeMethods(t *testing.T) {
	...
	p := sql.MakeTestPlanner()
	runParam := sql.NewRunParams(context.TODO(),
		p.ExtendedEvalContextCopy(), p)
...
func TestUpdateScheduleFunction(t *testing.T) {
	defer leaktest.AfterTest(t)()
	p := sql.MakeTestPlanner()
	runParam := sql.NewRunParams(context.TODO(), p.ExtendedEvalContextCopy(), p)
	schedule := &jobs.ScheduledJob{}
	require.NoError(t, sql.UpdateSchedule(*runParam, schedule))
}
...
func TestAlterStreamIntegration(t *testing.T) {
	...
	ctx := context.Background()
	params, _ := tests.CreateTestServerParams()
	params.Knobs.TSSchemaChanger = &ddl.TSSchemaChangerTestingKnobs{}
	s, db, _ := serverutils.StartServer(t, params)
	defer s.Stopper().Stop(ctx)
...
func TestAlterTSDatabaseNodeMethods(t *testing.T) {
	...
	node := ddl.NewAlterTSDatabaseNode(
