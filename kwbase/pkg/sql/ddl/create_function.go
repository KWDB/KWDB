...
var _ sql.PlanNode = &createFunctionNode{}
...
// NewCreateFunctionNode creates a new createFunctionNode. This func is added only for testing.
// nolint:unexportedreturn
func NewCreateFunctionNode(n *tree.CreateFunction, p *GenericPlanner) *createFunctionNode {
...
func CreateFunction(
	ctx context.Context, p *GenericPlanner, n *tree.CreateFunction,
) (sql.PlanNode, error) {
	...
	if err := sql.CheckUDFNameExists(p, string(n.FunctionName)); err != nil {
		return nil, err
	}
...
func (n *createFunctionNode) StartExec(params RunParams) error {
	...
	creator := params.GetPSessDataMutator().GetData().User
	...
	row := tree.Datums{
		...
		tree.NewDInt(tree.DInt(sqlconst.UDFFunctionDBID)),
		tree.NewDInt(tree.DInt(sqlconst.UDFFunctionSchemaID)),
...
func RegisterSQLFunction(params RunParams, fn *tree.SQLFunctionWrapper) error {
	...
	udfDef, err := sql.MakeSQLUDFDefinition(fn)
	if err != nil {
		return err
	}
