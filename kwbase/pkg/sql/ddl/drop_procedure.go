...
var _ sql.PlanNode = &dropProcedureNode{}
...
func DropProcedure(
	ctx context.Context, p *GenericPlanner, n *tree.DropProcedure,
) (sql.PlanNode, error) {
	found, desc, err := sql.ResolveProcedureObject(ctx, p, &n.Name)
	if err != nil {
		return nil, err
	}
	if !found {
...
func (d dropProcedureNode) StartExec(params RunParams) error {
	if err := dropProcedureImpl(params.Ctx, params.GetPlanner(), *d.procDesc); err != nil {
		return err
	}
	return nil
}
...
func dropProcedureImpl(
	ctx context.Context, p *GenericPlanner, procDesc sqlbase.ProcedureDescriptor,
) error {
	...
	for _, depID := range procDesc.DependsOn {
		...
		dependencyDesc.DependedOnBy = sql.RemoveMatchingReferences(dependencyDesc.DependedOnBy, procDesc.ID)
	...
	if err := removeProcedureComment(ctx, p, procDesc.ID); err != nil {
		return err
	}
...
func removeProcedureComment(ctx context.Context, p *GenericPlanner, procID sqlbase.ID) error {
