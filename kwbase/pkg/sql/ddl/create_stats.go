...
// CreateStatistics handles CREATE STATISTICS statements for query optimization
func CreateStatistics(
	ctx context.Context, p *GenericPlanner, n *tree.CreateStats,
) (sql.PlanNode, error) {
...
// framework.
var _ sql.PlanNode = &createStatsNode{}
...
// NewCreateStatsNode creates a new createStatsNode. This func is added only for testing.
// nolint:unexportedreturn
func NewCreateStatsNode(
	createStats tree.CreateStats, p *GenericPlanner, run createStatsRun,
) *createStatsNode {
	return &createStatsNode{
...
// NewCreateStatsNode2 creates a new createStatsNode.
// nolint:unexportedreturn
func NewCreateStatsNode2(createStats tree.CreateStats) *createStatsNode {
	return &createStatsNode{
		CreateStats: createStats,
		p:           nil,
		run:         createStatsRun{resultsCh: nil, errCh: nil},
	}
}
...
func (n *createStatsNode) makeJobRecord(ctx context.Context) (*jobs.Record, error) {
	...
	case *tree.UnresolvedObjectName:
		tableDesc, err = n.p.ResolveExistingObjectEx(ctx, t, true /*required*/, sql.ResolveRequireTableDesc)
		if err != nil {
			return nil, err
		}
	...
	case *tree.TableRef:
		...
		fqTableName, err = sql.GetQualifiedTableName(ctx, n.p.Txn(), &tableDesc.TableDescriptor)
		if err != nil {
			return nil, err
		}
	}
...
func (r *createStatsResumer) Resume(
	...
	if err := p.ExecCfg().DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		if details.AsOf != nil {
			p.GetSemaCtx().AsOfTimestamp = details.AsOf
		...
		if err := dsp.PlanAndRunCreateStats(
			ctx, evalCtx, planCtx, txn, r.job, sql.NewRowResultWriter(rows),
		); err != nil {
	...
	// eventlog.MakeEventLogger from the distsqlrun package.
	return evalCtx.ExecCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		return eventlog.MakeEventLogger(evalCtx.ExecCfg).InsertEventRecord(
			ctx,
			txn,
			eventlog.EventLogCreateStatistics,
...
func checkRunningJobs(ctx context.Context, job *jobs.Job, p *GenericPlanner) error {
	var jobID int64
	if job != nil {
		jobID = *job.ID()
	}
