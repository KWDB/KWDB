...
func TestFormatQuoteName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sqlutil.FormatQuoteNames(&buf, tt.names...)
...
func TestShowCreateView(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := sqlutil.ShowCreateView(context.Background(), &tn, tt.desc)
			if tt.expectedErr {
				require.Error(t, err)
			} else {
...
func TestShowCreateSequence(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tn := tree.Name(tt.tableName)
			result, err := sqlutil.ShowCreateSequence(context.Background(), &tn, tt.desc)
			if tt.expectedErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestShowCreateInstanceTable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := sqlutil.ShowCreateInstanceTable(sTable, tt.cTable, tt.tagName, tt.tagValue, tt.typ)
			require.Equal(t, tt.expected, result)
		})
	}
}
...
func TestShowCreatePartitioning(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := sqlutil.ShowCreatePartitioning(a, tt.tableDesc, tt.idxDesc, tt.partDesc, &buf, tt.indent, tt.colOffset)
			if tt.expectErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestShowCreateDisplayOptions(t *testing.T) {
	defer leaktest.AfterTest(t)()
	// 测试默认的显示选项
	defaultOpts := ddlopts.ShowCreateDisplayOptions{
		FKDisplayMode:  ddlopts.IncludeFkClausesInCreate,
		IgnoreComments: false,
	}

	require.Equal(t, ddlopts.IncludeFkClausesInCreate, defaultOpts.FKDisplayMode)
	require.False(t, defaultOpts.IgnoreComments)

	// 测试 OmitFKClausesFromCreate 模式
	omitOpts := ddlopts.ShowCreateDisplayOptions{
		FKDisplayMode:  ddlopts.OmitFKClausesFromCreate,
		IgnoreComments: true,
	}

	require.Equal(t, ddlopts.OmitFKClausesFromCreate, omitOpts.FKDisplayMode)
	require.True(t, omitOpts.IgnoreComments)

	// 测试 OmitMissingFKClausesFromCreate 模式
	omitMissingOpts := ddlopts.ShowCreateDisplayOptions{
		FKDisplayMode:  ddlopts.OmitMissingFKClausesFromCreate,
		IgnoreComments: false,
	}

	require.Equal(t, ddlopts.OmitMissingFKClausesFromCreate, omitMissingOpts.FKDisplayMode)
