...
var _ sql.PlanNode = &dropScheduleNode{}
var _ sql.PlanNodeFastPath = &dropScheduleNode{}
...
func DropSchedule(ctx context.Context, p *GenericPlanner, n *tree.DropSchedule) (PlanNode, error) {
	...
	if n.ScheduleName != nil {
		if *n.ScheduleName == sqlconst.ScheduleRetention ||
			*n.ScheduleName == sqlconst.ScheduleAutonomy || *n.ScheduleName == sqlconst.ScheduleVacuum || *n.ScheduleName == sqlconst.ScheduleCount {
	...
	if n.Schedules != nil {
		...
		for _, row := range rows {
			...
			if name == sqlconst.ScheduleRetention || name == sqlconst.ScheduleAutonomy || name == sqlconst.ScheduleVacuum || name == sqlconst.ScheduleCount {
...
func (n *dropScheduleNode) StartExec(params RunParams) error {
	for _, name := range n.scheduleNames {
		schedule, err := sql.LoadSchedule(params, name)
		if err != nil {
			return err
		}
		if schedule == nil {
		...
		err = sql.DeleteSchedule(params, schedule.ScheduleID())
		if err != nil {
			return err
		}
		n.numRow++
