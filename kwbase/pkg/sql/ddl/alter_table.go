...
var _ sql.PlanNode = &alterTableNode{}
var _ sql.PlanNodeReadingOwnWrites = &alterTableNode{}
...
// NewAlterTableNode creates a new alterTableNode. This func is added only for testing.
// nolint:unexportedreturn
func NewAlterTableNode(
	n *tree.AlterTable, tableDesc *MutableTableDescriptor, statsData map[int]tree.TypedExpr,
) *alterTableNode {
...
func AlterTable(ctx context.Context, p *GenericPlanner, n *tree.AlterTable) (sql.PlanNode, error) {
	tableDesc, err := p.ResolveMutableTableDescriptorEx(
		ctx, n.Table, !n.IfExists, sql.ResolveRequireTableDesc,
	)
	if errors.Is(err, sqlerror.ErrNoPrimaryKey) {
	...
	if !p.ExtendedEvalContext().TxnImplicit && tableDesc.IsTSTable() {
	...
	for i, cmd := range n.Cmds {
		...
		typedExpr, err := sql.AnalyzeExprWrap(ctx,
			p, injectStats.Stats,
	...
	if err = sql.CheckTableUsedByStream(ctx, p, uint64(tableDesc.ID), tableDesc.Name, n.Cmds, false); err != nil {
		return nil, err
	}
...
func (n *alterTableNode) StartExec(params RunParams) error {
	...
	tn := params.GetPlanner().ResolvedName(n.n.Table)
	dbName := tn.Catalog()
	...
	if n.tableDesc.IsTSTable() {
		var err error
		if pipeMetadatas, err = sql.CheckTableRelatedPipe(
			params.Ctx, params.GetPlanner(), uint64(n.tableDesc.ID), uint64(n.tableDesc.ParentID),
		); err != nil {
			return err
		}
	}
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableAddColumn:
			...
			if n.tableDesc.IsTSTable() {
				...
				if !n.tableDesc.IsSparseTable() && dataColNum+1 > sqlconst.MaxTSDataColumns {
					return pgerror.Newf(pgcode.TooManyColumns,
						"on timeseries table %s, the number of columns exceeded the maximum value %d", n.tableDesc.Name, sqlconst.MaxTSDataColumns)
				}
				if n.tableDesc.IsSparseTable() && dataColNum+1 > sqlconst.MaxSparseTSDataColumns {
					return pgerror.Newf(pgcode.TooManyColumns,
						"on sparse timeseries table %s, the number of columns exceeded the maximum value %d", n.tableDesc.Name, sqlconst.MaxSparseTSDataColumns)
				}
				...
				TSColumn, _, err := sqlbase.MakeTSColumnDefDescs(string(d.Name), d.Type, true, sqlbase.ColumnType_TYPE_DATA, d.DefaultExpr.Expr, params.GetPSemaCtx(), compressInfo)
				if err != nil {
					return err
				}
				...
				if len(pipeMetadatas) > 0 {
					...
					if err = sql.SendDDLToPipe(params, dbName, schemaName, tableName, sqlconst.KafkaMsgKindAlterTable, stmt, pipeMetadatas, false); err != nil {
						return err
					}
				}
			...
			newDef, seqDbDesc, seqName, seqOpts, err := sql.ProcessSerialInColumnDef(params.Ctx, params.GetPlanner(), d, tn, n.tableDesc.IsTSTable())
			if err != nil {
				return err
			}
			if seqName != nil {
				if err := sql.DoCreateSequence(
					params,
					n.n.String(),
					seqDbDesc,
			...
			d = newDef
			sql.IncTelemetryForNewColumn(d)

			col, idx, expr, err := sqlbase.MakeColumnDefDescs(d, params.GetPSemaCtx(), false)
			if err != nil {
				return err
			}
			...
			if d.HasDefaultExpr() {
				changedSeqDescs, err := sql.MaybeAddSequenceDependencies(
					params.Ctx, params.GetPlanner(), n.tableDesc, col, expr, nil,
				)
				if err != nil {
					return err
				}
			...
			if !col.Nullable && (col.DefaultExpr == nil && !col.IsComputed()) {
				kvs, err := params.PlannerTxn().Scan(params.Ctx, n.tableDesc.PrimaryIndexSpan().Key, n.tableDesc.PrimaryIndexSpan().EndKey, 1)
				if err != nil {
					return err
				}
				if len(kvs) > 0 {
			...
			if d.IsComputed() {
				if err := schema.ValidateComputedColumn(n.tableDesc, d, params.GetPSemaCtx()); err != nil {
					return err
				}
			}
		...
		case *tree.AlterTableAddConstraint:
			...
			case *tree.UniqueConstraintTableDef:
				if d.PrimaryKey {
					...
					if err := AlterPrimaryKey(params.Ctx, params.GetPlanner(), n.tableDesc, alterPK); err != nil {
						return err
					}
					continue
				}
				...
				if d.PartitionBy != nil {
					partitioning, err := sql.NewPartitioningDescriptor(
						params.Ctx,
			...
			case *tree.CheckConstraintTableDef:
				ck, err := sql.MakeCheckConstraint(params.Ctx,
					n.tableDesc, d, inuseNames, params.GetPSemaCtx(), *tn)
				if err != nil {
					return err
				}
			...
			case *tree.ForeignKeyConstraintTableDef:
				...
				affected := make(map[sqlbase.ID]*MutableTableDescriptor)
				...
				sql.RunWithOptionsWrap(params.GetPlanner(), sql.ResolveFlags{SkipCache: true}, func() {
					...
					kvs, scanErr := params.PlannerTxn().Scan(params.Ctx, n.tableDesc.PrimaryIndexSpan().Key, n.tableDesc.PrimaryIndexSpan().EndKey, 1)
					if scanErr != nil {
						err = scanErr
						return
					}
					var tableState sqlconst.FKTableState
					if len(kvs) == 0 {
						tableState = sqlconst.EmptyTable
					} else {
						tableState = sqlconst.NonEmptyTable
					}
					err = sql.ResolveFK(params.Ctx, params.GetTxn(), params.GetPlanner(), n.tableDesc, d, affected, tableState, t.ValidationBehavior, params.ExecCfg().Settings)
				})
				if err != nil {
					return err
				}
				...
				// if err := n.tableDesc.Validate(params.Ctx, params.PlannerTxn()); err != nil {
				// 	return err
				// }
		...
		case *tree.AlterTableAlterPrimaryKey:
			...
			if err := AlterPrimaryKey(params.Ctx, params.GetPlanner(), n.tableDesc, t); err != nil {
				return err
			}
		...
		case *tree.AlterTableDropColumn:
			...
			if len(colToDrop.UsesSequenceIds) > 0 {
				if err := params.GetPlanner().RemoveSequenceDependencies(params.Ctx, n.tableDesc, colToDrop); err != nil {
					return err
				}
			}
			...
			// by another column
			if err := canRemoveAllColumnOwnedSequences(params.Ctx, params.GetPlanner(), n.tableDesc, colToDrop, t.DropBehavior); err != nil {
				return err
			}

			if err := sql.DropSequencesOwnedByCol(params.Ctx, params.GetPlanner(), colToDrop, true /* queueJob */); err != nil {
				return err
			}
			...
			for _, ref := range n.tableDesc.DependedOnBy {
				...
				err := sql.CanRemoveDependentViewGeneric(
					params.Ctx, params.GetPlanner(), "column", string(t.Column), n.tableDesc.ParentID, ref, t.DropBehavior,
				)
				if err != nil {
					return err
				}
				viewDesc, skipped, err := sql.GetViewDescForCascade(
					params.Ctx, params.GetPlanner(), "column", string(t.Column), n.tableDesc.ParentID, ref.ID, t.DropBehavior,
				)
				if err != nil {
					return err
				}
				if skipped {
					...
					n.tableDesc.DependedOnBy = sql.RemoveMatchingReferences(n.tableDesc.DependedOnBy, ref.ID)
					continue
				}
				jobDesc := fmt.Sprintf("removing view %q dependent on column %q which is being dropped",
					viewDesc.Name, colToDrop.ColName())
				droppedViews, err = sql.RemoveDependentView(params.Ctx, params.GetPlanner(), n.tableDesc, viewDesc, jobDesc)
				if err != nil {
					return err
				}
			}
				...
				if containsThisColumn {
					if containsOnlyThisColumn || t.DropBehavior == tree.DropCascade {
						jobDesc := fmt.Sprintf("removing index %q dependent on column %q which is being"+
							...
							tree.AsStringWithFQNames(n.n, params.Ann()))
						if err := sql.DropIndexByName(
							params.Ctx, params.GetPlanner(), tn, tree.UnrestrictedName(idx.Name), n.tableDesc, false,
							t.DropBehavior, sqlconst.IgnoreIdxConstraint, jobDesc,
						); err != nil {
							return err
						}
					} else {
			...
			if err := sql.RemoveColumnComment(
				params.Ctx,
				params.GetPlanner(),
				params.PlannerTxn(),
				n.tableDesc.ID,
		...
		case *tree.AlterTableDropConstraint:
			...
			if err := n.tableDesc.DropConstraint(
				params.Ctx,
				name, details,
				func(desc *MutableTableDescriptor, ref *sqlbase.ForeignKeyConstraint) error {
					return sql.RemoveFKBackReferenceWrap(params.Ctx, params.GetPlanner(), desc, ref)
		...
		case *tree.AlterTableValidateConstraint:
			...
			case sqlbase.ConstraintTypeCheck:
				...
				if err := sql.ValidateCheckInTxn(
					params.Ctx, params.GetPlanner().LeaseMgr(), params.EvalContext(), n.tableDesc, params.EvalContext().Txn, ck.Expr,
				); err != nil {
					return err
				}
			...
			case sqlbase.ConstraintTypeFK:
				...
				if err := sql.ValidateFkInTxn(
					params.Ctx, params.GetPlanner().LeaseMgr(), params.EvalContext(), n.tableDesc, params.EvalContext().Txn, name,
				); err != nil {
					return err
				}
		...
		case *tree.AlterTableAlterTagType:
			...
			alteringTag, _, err := sqlbase.MakeTSColumnDefDescs(tagColumn.Name, newType, tagColumn.Nullable, sqlbase.ColumnType_TYPE_TAG, nil, params.GetPSemaCtx(), sqlbase.CompressInfo{})
			if err != nil {
				return err
			}
			...
			// Create a Job to perform the second stage of ts DDL.
			syncDetail := jobspb.SyncMetaCacheDetails{
				Type:         sql.AlterKwdbAlterTagType,
				SNTable:      n.tableDesc.TableDescriptor,
		...
		/*case *tree.AlterTableSetAudit:
		  var err error
		  descriptorChanged, err = params.GetPlanner().setAuditMode(params.Ctx, n.tableDesc.TableDesc(), t.Mode)
		  if err != nil {
		  	return err
		  }*/
		...
		case *tree.AlterTableInjectStats:
			...
			if !params.GetPlanner().EvalContext().TxnImplicit {
		...
		case *tree.AlterTableRenameColumn:
			...
			descChanged, err := renameColumn(params.Ctx, params.GetPlanner(), n.tableDesc,
		...
		case *tree.AlterTableRenameConstraint:
			...
			if err := params.GetPlanner().CheckPrivilege(params.Ctx, n.tableDesc, privilege.CREATE); err != nil {
				return err
			}

			depViewRenameError := func(objType string, refTableID sqlbase.ID) error {
				return dependentViewRenameError(params.Ctx, params.GetPlanner(),
			...
			if err := n.tableDesc.RenameConstraint(
				details, string(t.Constraint), string(t.NewName), depViewRenameError, func(desc *MutableTableDescriptor, ref *sqlbase.ForeignKeyConstraint, newName string) error {
					return updateFKBackReferenceName(params.Ctx, params.GetPlanner(), desc, ref, newName)
				}); err != nil {
				return err
			}
			descriptorChanged = true
		...
		case *tree.AlterTableAddTag:
			...
			if len(string(t.Tag.TagName)) > sqlconst.MaxTagNameLength {
				return sqlbase.NewTSNameOutOfLengthError("tag", string(t.Tag.TagName), sqlconst.MaxTagNameLength)
			}
			if !t.Tag.Nullable {
			...
			// Tag type check
			tagType, err := sql.CheckTagType(t.Tag.TagName, t.Tag.TagType)
			if err != nil {
				return err
			}
			t.Tag.TagType = tagType
			tagCol, _, err := sqlbase.MakeTSColumnDefDescs(string(t.Tag.TagName), t.Tag.TagType, t.Tag.Nullable, sqlbase.ColumnType_TYPE_TAG, nil, params.GetPSemaCtx(), sqlbase.CompressInfo{})
			if err != nil {
				return err
			}
			if tagCol.ID == 0 {
			...
			// Create a Job to perform the second stage of ts DDL.
			syncDetail := jobspb.SyncMetaCacheDetails{
				Type:       sql.AlterKwdbAddTag,
				SNTable:    n.tableDesc.TableDescriptor,
				AlterTag:   *tagCol,
				MutationID: mutationID,
			}
			stmt := tree.AsStringWithFQNames(n.n, params.Ann())
			jobID, err := sql.CreateTSSchemaChangeJob(params.Ctx, params.GetPlanner(), syncDetail, stmt, params.PlannerTxn())
			if err != nil {
				log.Info(params.Ctx, err)
			}
			if mutationID != sqlbase.InvalidMutationID {
		...
		case *tree.AlterTableDropTag:
			...
			for _, ref := range n.tableDesc.DependedOnBy {
				...
				err := sql.CanRemoveDependentViewGeneric(params.Ctx, params.GetPlanner(),
					"tag", string(t.TagName), n.tableDesc.ParentID, ref, tree.DropDefault,
				)
				if err != nil {
					return err
				}
				viewDesc, skipped, err := sql.GetViewDescForCascade(params.Ctx, params.GetPlanner(),
					"tag", string(t.TagName), n.tableDesc.ParentID, ref.ID, tree.DropDefault,
				)
				if err != nil {
					return err
				}
		...
		case *tree.AlterTableSetTag:
			...
			db, err := sql.GetDatabaseDescByID(params.Ctx, params.PlannerTxn(), n.tableDesc.ParentID)
			if err != nil {
				return err
			}
			insTable, found, err := sqlbase.ResolveInstanceName(params.Ctx, params.PlannerTxn(), db.Name, n.n.Table.Parts[0])
			if err != nil {
				return err
			} else if !found {
			...
			for _, name := range t.Expr.Names {
				...
				datum, err := sql.CheckTagValue(params, t.Expr.Expr, col.Type, col.Nullable, col.Name)
				if err != nil {
					return err
				}
			...
			if err := sql.WriteInstTableMeta(params.Ctx, params.PlannerTxn(), []sqlbase.InstNameSpace{insTable}, true); err != nil {
				return err
			}
		...
		case *tree.AlterTableSetRetentions:
			...
			lifeTime := sqlutil.GetTimeFromTimeInput(t.TimeInput)
			if lifeTime < 0 || lifeTime > sqlconst.MaxLifeTime {
			...
			downsampling = append(downsampling, sqlutil.TimeInputToString(t.TimeInput))
			n.tableDesc.TsTable.Downsampling = downsampling
			//descriptorChanged = true
			// Create a Job to perform the second stage of ts DDL.
			syncDetail := jobspb.SyncMetaCacheDetails{
				Type:    sql.AlterKwdbAlterRetentions,
				SNTable: n.tableDesc.TableDescriptor,
			}
			jobID, err := sql.CreateTSSchemaChangeJob(params.Ctx, params.GetPlanner(), syncDetail, tree.AsStringWithFQNames(n.n, params.Ann()), params.PlannerTxn())
			if err != nil {
		...
		case *tree.AlterPartitionInterval:
			...
			partitionInterval := sqlutil.GetTimeFromTimeInput(t.TimeInput)
			...
			partitionIntervalInput := sqlutil.TimeInputToString(t.TimeInput)
			...
			// Create a Job to perform the second stage of ts DDL.
			syncDetail := jobspb.SyncMetaCacheDetails{
				Type:    sql.AlterKwdbAlterPartitionInterval,
				SNTable: n.tableDesc.TableDescriptor,
			}
			jobID, err := sql.CreateTSSchemaChangeJob(params.Ctx, params.GetPlanner(), syncDetail, tree.AsStringWithFQNames(n.n, params.Ann()), params.PlannerTxn())
			if err != nil {
	...
	// update.
	params.GetPlanner().SetAuditTarget(uint32(n.tableDesc.GetID()), n.tableDesc.GetName(), droppedViews)
	...
	if n.tableDesc.IsTable() && len(pipeMetadatas) > 0 {
		...
		stmt := tree.AsStringWithFQNames(n.n, params.Ann())
		if err := sql.SendDDLToPipe(params, dbName, schemaName, tableName, sqlconst.KafkaMsgKindAlterTable, stmt, pipeMetadatas, true); err != nil {
			return err
		}
	}
	return nil
...
/*
func (p *GenericPlanner) setAuditMode(
	ctx context.Context, desc *TableDescriptor, auditMode tree.AuditMode,
) (bool, error) {
...
func addIndexMutationWithSpecificPrimaryKey(
	table *MutableTableDescriptor, toAdd *IndexDescriptor, primary *IndexDescriptor,
) error {
...
func applyColumnMutation(
	...
	case *tree.AlterTableAlterColumnType:
		...
		if tableDesc.IsTSTable() {
			...
			if !isOnlyMetaChanged {
				...
				alteringCol, _, err := sqlbase.MakeTSColumnDefDescs(col.Name, newType, col.Nullable, sqlbase.ColumnType_TYPE_DATA, nil, params.GetPSemaCtx(), compressInfo)
				if err != nil {
					return false, err
				}
				...
				// Create a Job to perform the second stage of ts DDL.
				syncDetail := jobspb.SyncMetaCacheDetails{
					Type:            sql.AlterKwdbAlterColumnType,
				...
				jobID, err := sql.CreateTSSchemaChangeJob(params.Ctx, params.GetPlanner(), syncDetail, params.GetPSQL(), params.PlannerTxn())
				if err != nil {
					return false, err
				}
				if mutationID != sqlbase.InvalidMutationID {
				...
				if err = sql.WriteTableDesc(params.Ctx, params.GetPlanner(), tableDesc); err != nil {
					return false, err
				}
				// Actively commit a transaction, and read/write system table operations
				// need to be performed before this.
		...
		version := params.ExecCfg().Settings.Version.ActiveVersionOrEmpty(params.Ctx)
		if supported, err := sqlutil.IsTypeSupportedInVersion(version, typ); err != nil {
			return false, err
		} else if !supported {
			return false, pgerror.Newf(
				pgcode.FeatureNotSupported,
	...
	case *tree.AlterTableSetDefault:
		...
		if len(col.UsesSequenceIds) > 0 {
			if err := params.GetPlanner().RemoveSequenceDependencies(params.Ctx, tableDesc, col); err != nil {
				return false, err
			}
		}
		if t.Default == nil {
			...
			expr, err := sqlbase.SanitizeVarFreeExpr(
				t.Default, colDatumType, "DEFAULT", params.GetPSemaCtx(), true /* allowImpure */, isTSTable, col.Name,
			)
			if err != nil {
				return false, err
			}
			...
			changedSeqDescs, err := sql.MaybeAddSequenceDependencies(
				params.Ctx, params.GetPlanner(), tableDesc, col, expr, nil, /* backrefs */
			)
			if err != nil {
				return false, err
			}
...
func castDefaultFuncExpr(
	...
	if tmp, ok := parsedExpr.(*tree.AnnotateTypeExpr); ok {
		if innerExpr, isFunc := tmp.TypedInnerExpr().(*tree.FuncExpr); isFunc {
			newDefaultExpr, err := innerExpr.TypeCheck(params.GetPSemaCtx(), typ)
			if err != nil {
				return err
			}
...
// data.
func injectTableStats(params RunParams, desc *TableDescriptor, statsExpr tree.TypedExpr) error {
	...
	if _ /* rows */, err := params.ExecCfg().InternalExecutor.Exec(
		params.Ctx,
		"delete-stats",
	...
	params.ExecCfg().TableStatsCache.InvalidateTableStats(params.Ctx, desc.ID)

	// Use Gossip to refresh the caches on other nodes.
	return stats.GossipTableStatAdded(params.ExecCfg().Gossip, desc.ID)
}
...
func updateFKBackReferenceName(
	ctx context.Context,
	p *GenericPlanner,
	tableDesc *MutableTableDescriptor,
...
func validateTextType(colName string, oldType, newType *types.T, colType sqlbase.ColumnType) error {
	...
	switch newType.Oid() {
	case oid.T_bpchar:
		if newType.Width() >= sqlconst.MaxFixedLen {
			return newTypeExceedError(colName, newType, sqlconst.MaxFixedLen)
		}
	...
	case types.T_nchar:
		if newType.Width() > sqlconst.MaxNCharLen {
			return newTypeExceedError(colName, newType, sqlconst.MaxNCharLen)
		}
		if oldType.Oid() == oid.T_bpchar || oldType.Oid() == oid.T_varchar {
...
func prepareAlterType(t *types.T) *types.T {
	...
	case oid.T_varchar:
		if t.InternalType.Width == 0 {
			return types.MakeVarChar(sqlconst.DefaultVariableLEN, t.TypeEngine())
		}
	case types.T_nvarchar:
		if t.InternalType.Width == 0 {
...
func checkTableRelatedPubsAndSubs(
	...
	_, pubTableMap, err := sql.FetchAllPublishedObjects(ctx, p, dbDesc)
	if err != nil {
		return err
	}
...
func columnUsedInIndex(tableDesc *MutableTableDescriptor, colID sqlbase.ColumnID) bool {
...
func indexDescriptorContainsColumnID(idx *IndexDescriptor, colID sqlbase.ColumnID) bool {
	for _, id := range idx.ColumnIDs {
		if id == colID {
			return true
		}
