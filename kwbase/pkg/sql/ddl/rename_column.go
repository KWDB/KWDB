...
var _ sql.PlanNode = &renameColumnNode{}
...
func RenameColumn(
	ctx context.Context, p *GenericPlanner, n *tree.RenameColumn,
) (sql.PlanNode, error) {
	// Check if table exists.
	tableDesc, err := p.ResolveMutableTableDescriptor(ctx, &n.Table, !n.IfExists, sql.ResolveRequireTableDesc)
	if err != nil {
		return nil, err
	}
	if tableDesc == nil {
...
func (n *renameColumnNode) StartExec(params RunParams) error {
	...
	descChanged, err := renameColumn(params.Ctx, params.GetPlanner(), tableDesc, &n.n.Name,
...
func renameColumn(
	...
	if tableDesc.IsTSTable() {
		if len(string(*newName)) > sqlconst.MaxTagNameLength {
			return false, sqlbase.NewTSNameOutOfLengthError("column/tag", string(*newName), sqlconst.MaxTagNameLength)
		}
	...
	for _, tableRef := range tableDesc.DependedOnBy {
		...
		if found {
			return false, dependentViewRenameError(ctx,
				p, typName, oldName.String(), tableDesc.ParentID, tableRef.ID)
		}
	}
	if *oldName == *newName {
		// Noop.
	...
	for oldShardColName, newShardColName := range shardColumnsToRename {
		...
		_, err = renameColumn(ctx, p, tableDesc, &oldShardColName, &newShardColName,
