...
func runTimeSeriesImport(
	...
	if err == nil {
		...
		if flowCtx.EvalCtx.StartSinglenode {
			if err := flowCtx.Cfg.GetTSEngine().TSFlushVGroups(); err != nil {
				return nil, err
			}
		} else {
...
func (t *timeSeriesImportInfo) ingest(
	...
	if t.flowCtx.EvalCtx.StartSinglenode && t.flowCtx.EvalCtx.Kwengineversion == "1" {
		...
		resp, _, err := t.flowCtx.Cfg.GetTSEngine().PutData(uint64(t.tbID), [][]byte{payload}, uint64(0), t.writeWAL, nil)
		if err != nil {
			for i := range datums {
				cols := datums[i]
				rowString := tree.ConvertDatumsToStr(cols, ',')
	...
	if t.flowCtx.EvalCtx.StartSinglenode && t.flowCtx.EvalCtx.Kwengineversion == "2" {
		for _, val := range payloadNodeMap[int(t.flowCtx.EvalCtx.NodeID)].PerNodePayloads {
			resp, _, err := t.flowCtx.Cfg.GetTSEngine().PutData(uint64(t.tbID), [][]byte{val.Payload}, uint64(0), t.writeWAL, nil)
			if err != nil {
				for i := range datums {
					cols := datums[i]
					rowString := tree.ConvertDatumsToStr(cols, ',')
