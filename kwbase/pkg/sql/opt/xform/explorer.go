...
func (e *explorer) lookupExploreState(grp memo.RelExpr) *exploreState {
	// To explore SortExpr, use Input.FirstExpr to retrieve the exploration result.
	if s, ok := grp.(*memo.SortExpr); ok {
		return &e.o.lookupOptState(s.FirstExpr(), physical.MinRequired).explore
	}
