...
const (
	// OrderFromSubquerySave Saved Order from the fromSubquery
	OrderFromSubquerySave = 1 << 0
)
...
func (oc *OrderingChoice) CheckFlag(flag int) bool {
	return oc.OrderFlags&flag > 0
}

// SetFlag set the flag
func (oc *OrderingChoice) SetFlag(flag int) {
	oc.OrderFlags |= flag
}

// ClearFlag clear the flag
func (oc *OrderingChoice) ClearFlag(flag int) {
	oc.OrderFlags &= ^flag
}
