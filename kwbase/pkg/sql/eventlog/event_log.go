...
func init() {
	sql.EventLogSetClusterSettingType = string(EventLogSetClusterSetting)
	sql.InsertEventRecordFunc = func(
		ctx context.Context,
		...
		return MakeEventLogger(execCfg).InsertEventRecord(
			ctx, txn, EventLogType(eventType), targetID, reportingID, info,
		)
	}
}

// EventLogType represents an event type that can be recorded in the event log.
type EventLogType string // nolint
...
const (
	...
	// eventlog.EventLogCommentOnColumn is recorded when a column is commented.
	EventLogCommentOnColumn EventLogType = "comment_on_column"
	// eventlog.EventLogCommentOnTable is recorded when a table is commented.
	EventLogCommentOnDatabase EventLogType = "comment_on_database"
	// eventlog.EventLogCommentOnTable is recorded when a table is commented.
	EventLogCommentOnTable EventLogType = "comment_on_table"
	// eventlog.EventLogCommentOnIndex is recorded when a index is commented.
...
func MakeEventLogger(execCfg *sql.ExecutorConfig) EventLogger {
