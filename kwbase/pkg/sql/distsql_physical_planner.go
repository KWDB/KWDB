...
func GetEngine(planCtx *PlanningCtx) execinfrapb.ProcessorSpecEngineType {
	if planCtx.apSelect {
		return execinfrapb.ProcessorSpec_Analytical
	}
	return execinfrapb.ProcessorSpec_Relation
}
...
func (dsp *DistSQLPlanner) createTSDDL(planCtx *PlanningCtx, n *tsDDLNode) (PhysicalPlan, error) {
	...
	for i := 0; i < len(n.nodeID); i++ {
		...
		case dropKwdbAPTable:
			...
			apDropTable.DbName = n.d.Database.Name
