...
create table ts_db.t4 (k_timestamp TIMESTAMPTZ(3) NOT NULL, e1 INT8 NULL) tags(code int not null) primary tags(code);

insert into ts_db.t4 values('2002-02-22 10:48:12.899+00:00', 9223372036854775807, 1);

SELECT k_timestamp,CAST(e1 AS TIMESTAMPTZ) FROM ts_db.t4;

create table ts_db.t6(k_timestamp timestamp not null, e1 int) tags(a int not null) primary tags(a);
insert into ts_db.t6 values(now(), 2147483647, 1);
...
insert into test_select_timebucket_ms.tb values('0000-02-16 17:10:22.123','0000-11-01 08:30:00.867',600,6000,60000,760000.767,600000.6060606,true,'w',null,null,null,'test---->测试',null,'测试中文testing-----',null,null,null,null,null,null,null,'中文测试abcwww+++',1,null,6000,true,66.66666,500.56756,'t','test测试！！！@TEST1','e','a',null,'test----->>>64_1','c','test----<>1023_1','test中文测试_1','uuuuwww');
