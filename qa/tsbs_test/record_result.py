...
parser.add_argument('--result_dir','-d',type=str, default="none",help='tsbs query result dir')
parser.add_argument('--threshold_dir','-c',type=str, default="none",help='threshold dir')
...
if __name__ == '__main__':
    ...
    print(f"场景 {str(scale)} {case_name} 测试结果文件：{result_file}")
    
    compare_num = 100000000
    if case_name == "load":
        ...
        if threshold_dir != "none":
            threshold = utils.get_threshold_from_csv(os.path.join(threshold_dir, 'TSBS_THRESHOLD.csv'), kwdb_version, tsbs_format, pipe_name, case_name, str(scale), str(workers), str(query_times))
            if threshold == None or compare_num < float(threshold):
                print(f"场景 {str(scale)} 写入 测试失败：阈值：threshold {threshold}, 测试结果： {compare_num}")
                exit(1)
    else:
        ...
        if threshold_dir != "none":
            threshold = utils.get_threshold_from_csv(os.path.join(threshold_dir, 'TSBS_THRESHOLD.csv'), kwdb_version, tsbs_format, pipe_name, case_name, str(scale), str(workers), str(query_times))
            if threshold == None or compare_num > float(threshold):
                print(f"场景 {str(scale)} {case_name} 测试失败：阈值：threshold {threshold}, 测试结果： {compare_num}")
                exit(1)