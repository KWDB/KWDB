...
unique_ptr<PhysicalPlan> DuckdbExec::CreateAPTableScan(const int *i) {
  ...
  if (proc.has_core() && proc.core().has_aptablereader()) {
    ...
    auto entry =
        schema.GetEntry(catalog->GetCatalogTransaction(*connect_->context), CatalogType::TABLE_ENTRY, table_name);
    auto &table = entry->Cast<TableCatalogEntry>();
    properties_->RegisterDBRead(table.ParentCatalog(), *connect_->context);
    ...
    auto &table_scan = physical_planner_->Make<PhysicalTableScan>(
    ...
    if (add_filter) {
      ...
      auto &res_proj = physical_planner_->Make<PhysicalProjection>(proj_types, std::move(proj_exprs), 0);
