...
PhyOpRef TransFormPlan::TransFormTableScan(const kwdbts::ProcessorSpec& procSpec,
  ...
  if (post.output_columns_size() > 0 && post.output_columns_size() != post.output_types_size()) {
    throw InternalException("post.output_columns_size() > 0 && post.output_columns_size() != post.output_types_size()");
  }
  std::string db_name;
  ...
  if (post.has_filter() && post.filter().has_expr() && !post.filter().expr().empty()) {
    plan = AddAPFilters(table_scan, post, table, col_map);
  }
  // add projection
