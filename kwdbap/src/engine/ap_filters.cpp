...
PhyPlanPtr TransFormPlan::VerifyProjectionByTableScan(PhyPlanPtr plan, IdxMap &col_map) {
  ...
  for (idx_t col_idx = 0; col_idx < column_count; col_idx++) {
    ...
    expressions[new_col_idx] = make_uniq<BoundReferenceExpression>(column_names[col_idx],
                                                                   child_types[col_idx],
                                                                   col_idx);
  ...
  for (idx_t col_idx = 0; col_idx < result_proj_column_count; col_idx++) {
    result_expressions.emplace_back(make_uniq<BoundConstantExpression>(Value(LogicalType::UTINYINT)));
  ...
  for (idx_t col_idx = 0; col_idx < column_count; col_idx++) {
    ...
    result_expressions[new_col_idx] = make_uniq<BoundReferenceExpression>(column_names[col_idx],
                                                                          child_types[col_idx],
                                                                          proj_idx);
