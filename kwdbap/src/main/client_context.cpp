...
shared_ptr<PreparedStatementData>
ClientContext::ConvertFlowToPhysicalPlan(ClientContextLock &lock, const string &query, const string &database, const string &table, string flowSpecs, PreparedStatementMode mode) {
    ...
    result->physical_plan = physical_planner.KWPlan(flows, in_table, properties, names);
...
unique_ptr<PendingQueryResult> ClientContext::KWPendingStatementInternal(ClientContextLock &lock, const string &query,
                                                                         shared_ptr<PreparedStatementData> &prepared,
                                                                         const PendingQueryParameters &parameters) {

   // auto res_prepared = ConvertFlowToPhysicalPlan(lock, query, database, table, "scan", PreparedStatementMode::PREPARE_AND_EXECUTE);
...
unique_ptr<PendingQueryResult> ClientContext::KWPendingStatementOrPreparedStatement(
        ClientContextLock &lock, const string &query, shared_ptr<PreparedStatementData> &prepared, const PendingQueryParameters &parameters) {
    unique_ptr<PendingQueryResult> pending;
    ...
    bool invalidate_query = true;
    try {
        pending = KWPendingStatementInternal(lock, query, prepared, parameters);
    } catch (std::exception &ex) {
        ErrorData error(ex);
        if (!Exception::InvalidatesTransaction(error.Type())) {
            // standard exceptions do not invalidate the current transaction
...
unique_ptr<QueryResult> ClientContext::KWQuery(const string &query, shared_ptr<PreparedStatementData> &prepared, bool allow_stream_result) {
  auto lock = LockContext();
  ...
  PendingQueryParameters parameters;
  parameters.allow_stream_result = allow_stream_result;
  auto pending_query = KWPendingStatementOrPreparedStatement(*lock, query, prepared, parameters);
  auto has_result = pending_query->properties.return_type == StatementReturnType::QUERY_RESULT;
  unique_ptr<QueryResult> current_result;
  if (pending_query->HasError()) {
    current_result = ErrorResult<MaterializedQueryResult>(pending_query->GetErrorObject());
