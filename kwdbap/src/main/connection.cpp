...
unique_ptr<MaterializedQueryResult> Connection::KWQuery(const string &query, shared_ptr<PreparedStatementData> &prepared) {
	auto result = context->KWQuery(query, prepared, false);
	D_ASSERT(result->type == QueryResultType::MATERIALIZED_RESULT);
	return unique_ptr_cast<QueryResult, MaterializedQueryResult>(std::move(result));
}
