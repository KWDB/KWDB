...
unique_ptr<PhysicalPlan> PhysicalPlanGenerator::KWPlan(vector<string> flowSpecs, TableCatalogEntry &table,
                                                       StatementProperties &properties, vector<string> &res_names) {
    if (!physical_plan) {
        physical_plan = make_uniq<PhysicalPlan>(Allocator::Get(context));
    }
    for (auto it = flowSpecs.rbegin(); it != flowSpecs.rend(); ++it) {
        if (*it == "scan") {
...
void PhysicalPlanGenerator::InitPhysicalPlan() {
    if (!physical_plan) {
        physical_plan = make_uniq<PhysicalPlan>(Allocator::Get(context));
    }
}

unique_ptr<PhysicalPlan> PhysicalPlanGenerator::GetPhysicalPlan() {
    return std::move(physical_plan);
}
