...
void timmerge(RandomAccessIterator first, RandomAccessIterator middle,
  ...
  timmerge(first, middle, last, compare, detail::identity());
}
...
void timmerge(RandomAccessIterator first, RandomAccessIterator middle,
  ...
  timmerge(first, middle, last, std::less<value_type>(), detail::identity());
}
...
void timsort(RandomAccessIterator const first, RandomAccessIterator const last, Compare compare) {
  timsort(first, last, compare, detail::identity());
}
...
void timsort(RandomAccessIterator const first, RandomAccessIterator const last) {
  typedef typename std::iterator_traits<RandomAccessIterator>::value_type value_type;
  timsort(first, last, std::less<value_type>(), detail::identity());
}
...
void timsort(RandomAccessRange &range, Compare compare) {
  timsort(std::begin(range), std::end(range), compare);
}
...
void timsort(RandomAccessRange &range) {
  timsort(std::begin(range), std::end(range));
}
