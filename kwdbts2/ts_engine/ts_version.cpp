  ...
  for (int l = max_level - 1; l >= 0; --l) {
    ...
    for (int g = 0; g < max_group; ++g) {
      ...
      if (need_compact) {
        *level = l;
        *group = g;
        auto res = leveled_last_segments_.GetLastSegments(l, g);
        if (res.size() > EngineOptions::max_compact_num) {
          res.resize(EngineOptions::max_compact_num);
        }
        return res;
      }
    }
...
KStatus TsPartitionVersion::GetBlockSpans(const TsScanFilterParams& filter,
                                          ...
                                          bool skip_mem, bool skip_last, bool skip_entity,
  ...
  if (!skip_last) {
    ...
    for (int level = 0; level < leveled_last_segments_.GetMaxLevel(); ++level) {
      ...
      for (const auto &last_seg : last_segments) {
        s = last_seg->GetBlockSpans(block_data_filter, *ts_block_spans, tbl_schema_mgr, scan_schema, cache, ts_scan_stats);
        if (s != KStatus::SUCCESS) {
