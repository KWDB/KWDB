...
//   TSSlice data_value{};
//   for (int i = 0; i < cnt; i++) {
//     data_value = GenSomePayloadData(ctx_, row_num_, start_ts1 + i * 100, &meta, 10);
//     // TSSlice payload1{data_value, p_len};
//     ASSERT_EQ(tbl_range->PutData(ctx_, data_value), KStatus::SUCCESS);
...
TEST_F(TestEngine, tagiterator) {
  ...
  TSSlice data_value{};
  data_value = GenSomePayloadData_V2Fixed(ctx_, row_num_, start_ts1, &meta, 10);

  TsRawPayload p{data_value};
  TSEntityID entity_id = 0;
  uint32_t tbl_grp_id = 0;
  bool new_tag = false;

  auto schema_mgr = ts_engine_->GetSchemaMgr();

  ASSERT_EQ(schema_mgr->GetVGroup(ctx_, TSTableID(cur_table_id), p.GetPrimaryTag(), 
    &tbl_grp_id, &entity_id, &new_tag), KStatus::SUCCESS);

  auto tbl_grp = ts_engine_->GetVGroupByID(ctx_, tbl_grp_id);

  if (new_tag) {
    entity_id = tbl_grp->AllocateEntityID();
    ASSERT_EQ(ts_engine_->PutTagData(ctx_, TSTableID(cur_table_id), tbl_grp_id, entity_id, p), KStatus::SUCCESS);
  }
...
//   for (int i = 0; i < cnt; i++) {
//     data_value = GenSomePayloadData(ctx_, row_num_, start_ts1 + i * 100, &meta, 10);
//     ASSERT_EQ(tbl_grp->PutData(ctx_, TSTableID(cur_table_id), entity_id, &data_value), KStatus::SUCCESS);
//     std::cout << "[DEBUG] PutData" << std::endl;
//     free(data_value.data);
//   }
  ASSERT_EQ(tbl_grp->PutData(ctx_, TSTableID(cur_table_id), entity_id, &data_value), KStatus::SUCCESS);
...
//     k_uint32 p_len = 0;
//     data_value = GenSomePayloadData(ctx_, row_num_, start_ts1 + i * 100, &meta, 10);
//     ASSERT_EQ(tbl_range->PutEntity(ctx_, data_value, 0), KStatus::SUCCESS);
//     free(data_value.data);
//   }
//   // tag iterator
//   std::vector<EntityResultIndex> entity_id_list;
...
//     data_value = GenSomePayloadData(ctx_, row_num_, start_ts1 + i * 100, &meta, 10);
//     KUint32(data_value.data + Payload::ts_version_offset_) = new_version;
//     ASSERT_EQ(tbl_range->PutData(ctx_, data_value), KStatus::SUCCESS);
//     free(data_value.data);
//   } 
