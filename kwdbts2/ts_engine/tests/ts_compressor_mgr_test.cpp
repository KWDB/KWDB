...
TYPED_TEST(CompressorManagerTester, TwoLevelCompress) {
  ...
  kwdbts::TsCompressionConfig cfg{GetCompressorAlg<TypeParam>::Alg, kwdbts::CompressAlgo::kSnappy, 1};
  ASSERT_TRUE(
      inst.CompressData({reinterpret_cast<char*>(vec.data()), vec.size() * sz}, nullptr, vec.size(), &out, cfg));
  ...
  ASSERT_TRUE(
      inst.CompressData({reinterpret_cast<char*>(vec.data()), vec.size() * sz}, &bitmap, vec.size(), &out, cfg));
...
TYPED_TEST(CompressorManagerTester, Null_Compressions) {
  ...
  for (auto p : probabilities) {
    ...
    kwdbts::TsCompressionConfig cfg{GetCompressorAlg<TypeParam>::Alg, kwdbts::CompressAlgo::kPlain, 0};
    bool ok = inst.CompressData(raw_slice, bitmap.get(), count, &compressed, cfg);
    ASSERT_TRUE(ok);
    ASSERT_GE(compressed.size(), sizeof(uint32_t));
    const size_t payload_size = compressed.size() - sizeof(uint32_t);
    const bool first_stage_enabled = kwdbts::EngineOptions::compress_stage & kwdbts::kEncodeEnableMask;
...
TEST(CompressorManager, ChimpAlgorithmMatchesFloatingType) {
  ...
  auto float_cfg = inst.GetCompConfig(/*table_id=*/0, attr_info);
  attr_info.type = DATATYPE::DOUBLE;
  auto double_cfg = inst.GetCompConfig(/*table_id=*/0, attr_info);

  EXPECT_EQ(float_cfg.encoder, kwdbts::EncodeAlgo::kChimp_32);
  EXPECT_EQ(double_cfg.encoder, kwdbts::EncodeAlgo::kChimp_64);
}
...
TEST(CompressorManager, InvalidCompressionLevelFallsBackToDefault) {
  ...
  kwdbts::TsBufferBuilder compressed;
  kwdbts::TsCompressionConfig cfg{kwdbts::EncodeAlgo::kSimple8B_V2_s32, kwdbts::CompressAlgo::kZstd, 99};
  ASSERT_TRUE(inst.CompressData({reinterpret_cast<char*>(vec.data()), vec.size() * sizeof(int32_t)}, nullptr,
                                vec.size(), &compressed, cfg));
  ...
  cfg = kwdbts::TsCompressionConfig{kwdbts::EncodeAlgo::kPlain, kwdbts::CompressAlgo::kZstd, -3};
  ASSERT_TRUE(inst.CompressVarchar({varchar.data(), varchar.size()}, &varchar_compressed, cfg));
...
TEST(CompressorManager, UnspecifiedCompressionLevelUsesClusterSetting) {
  ...
  for (const auto& tc : cases) {
    ...
    kwdbts::TsCompressionConfig cfg{kwdbts::EncodeAlgo::kSimple8B_V2_s32, kwdbts::CompressAlgo::kZstd,
                                    tc.explicit_level};
    ASSERT_TRUE(inst.CompressData({reinterpret_cast<char*>(vec.data()), vec.size() * sizeof(int32_t)}, nullptr,
                                  vec.size(), &expected_numeric, cfg));
    cfg.level = roachpb::COMPRESS_LEVEL_UNSPECIFIED;
    ASSERT_TRUE(inst.CompressData({reinterpret_cast<char*>(vec.data()), vec.size() * sizeof(int32_t)}, nullptr,
                                  vec.size(), &actual_numeric, cfg));
    ...
    cfg.level = static_cast<roachpb::ColumnCompressLevel>(tc.explicit_level);
    ASSERT_TRUE(inst.CompressVarchar({varchar.data(), varchar.size()}, &expected_varchar, cfg));
    cfg.level = roachpb::COMPRESS_LEVEL_UNSPECIFIED;
    ASSERT_TRUE(inst.CompressVarchar({varchar.data(), varchar.size()}, &actual_varchar, cfg));
...
TEST(CompressorManager, UnspecifiedCompressionAlgorithmUsesCurrentClusterSetting) {
  ...
  auto cfg0 = inst.GetCompConfig(/*table_id=*/0, attr_info);
  auto cfg_default0 = inst.GetDefaultCompConfigByType(DATATYPE::INT32);
  // GetCompConfig applies the compress_stage mask, so with stage=0 both
  // encoder and compressor are forced to Plain regardless of cluster defaults.
  EXPECT_EQ(cfg0.encoder, kwdbts::EncodeAlgo::kPlain);
  EXPECT_EQ(cfg0.compressor, kwdbts::CompressAlgo::kPlain);
  EXPECT_EQ(cfg_default0.encoder, kwdbts::EncodeAlgo::kSimple8B_V2_s32);
  EXPECT_EQ(cfg_default0.compressor, kwdbts::CompressAlgo::kPlain);

  kwdbts::EngineOptions::compress_stage = 3;
  kwdbts::EngineOptions::compression_algorithm = kwdbts::CompressAlgo::kZstd;
  auto cfg3 = inst.GetCompConfig(/*table_id=*/0, attr_info);
  auto cfg_default3 = inst.GetDefaultCompConfigByType(DATATYPE::INT32);
  EXPECT_EQ(cfg3.encoder, kwdbts::EncodeAlgo::kSimple8B_V2_s32);
  EXPECT_EQ(cfg3.compressor, kwdbts::CompressAlgo::kZstd);
  EXPECT_EQ(cfg_default3.encoder, kwdbts::EncodeAlgo::kSimple8B_V2_s32);
  EXPECT_EQ(cfg_default3.compressor, kwdbts::CompressAlgo::kZstd);

  attr_info.type = DATATYPE::TIMESTAMP64;
  auto ts_cfg = inst.GetCompConfig(/*table_id=*/0, attr_info);
  auto cfg_ts_default = inst.GetDefaultCompConfigByType(DATATYPE::TIMESTAMP64);
  EXPECT_EQ(ts_cfg.encoder, kwdbts::EncodeAlgo::kSimple8B_V2_s64);
  EXPECT_EQ(ts_cfg.compressor, kwdbts::CompressAlgo::kPlain);
  EXPECT_EQ(cfg_ts_default.encoder, kwdbts::EncodeAlgo::kSimple8B_V2_s64);
  EXPECT_EQ(cfg_ts_default.compressor, kwdbts::CompressAlgo::kPlain);
...
TEST(CompressorManager, VarcharCompressionFallsBackToPlainWhenPayloadGrows) {
  ...
  kwdbts::TsBufferBuilder compressed;
  kwdbts::TsCompressionConfig cfg{kwdbts::EncodeAlgo::kSimple8B_V2_s32, kwdbts::CompressAlgo::kLz4, 1};
  ASSERT_TRUE(inst.CompressVarchar({input.data(), input.size()}, &compressed, cfg));
...
TEST(CompressorManager, VarcharCompressionHonorsCompressStage) {
  ...
  kwdbts::TsCompressionConfig cfg{kwdbts::EncodeAlgo::kPlain, kwdbts::CompressAlgo::kPlain,
                                  roachpb::COMPRESS_LEVEL_UNSPECIFIED};
  ASSERT_TRUE(inst.CompressVarchar({input.data(), input.size()}, &plain, cfg));

  for (uint8_t stage : {static_cast<uint8_t>(0), static_cast<uint8_t>(1)}) {
    kwdbts::EngineOptions::compress_stage = stage;
    kwdbts::TsBufferBuilder compressed;
    cfg.compressor = kwdbts::CompressAlgo::kZstd;
    ASSERT_TRUE(inst.CompressVarchar({input.data(), input.size()}, &compressed, cfg));
