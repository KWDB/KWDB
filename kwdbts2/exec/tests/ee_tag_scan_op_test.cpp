...
// Tag scan operation test spec
class SpecTagScan : public SpecBase {
 public:
  explicit SpecTagScan(k_uint64 table_id) : SpecBase(table_id) {
  }
...
class TestTagScanOp : public OperatorTestBase {
 public:
  TestTagScanOp() : OperatorTestBase() {
  }
...
// Test tag scan operation
TEST_F(TestTagScanOp, TestDmlExecTagScan) {
  TSFlowSpec flow;
  SpecTagScan tag_scan_spec(table_id_);
  tag_scan_spec.PrepareFlowSpec(flow);
  tag_scan_spec.PrepareInputOutputSpec(flow);
