...
class TestWindowOp : public OperatorTestBase {
 public:
  TestWindowOp() : OperatorTestBase() {
  }
...
// Test window function with DIFF
TEST_F(TestWindowOp, TestDmlExecWindow) {
  TSFlowSpec flow;
  SpecWindow window_spec(table_id_);
  window_spec.PrepareFlowSpec(flow);
  window_spec.PrepareInputOutputSpec(flow);
