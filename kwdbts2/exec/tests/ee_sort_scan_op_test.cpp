...
// Sort scan operation test spec
class SpecSortScan : public SpecBase {
 public:
  explicit SpecSortScan(k_uint64 table_id) : SpecBase(table_id) {
  }
  ...
  void PrepareFlowSpec(TSFlowSpec& flow) override {
    ...
    // sort reader
    PrepareSortReaderProcessor(flow, 1);
...
class TestSortScanOp : public OperatorTestBase {
 public:
  TestSortScanOp() : OperatorTestBase() {
  }
...
// Test sort scan operation
TEST_F(TestSortScanOp, TestDmlExecSortScan) {
  TSFlowSpec flow;
  SpecSortScan sort_scan_spec(table_id_);
  sort_scan_spec.PrepareFlowSpec(flow);
  sort_scan_spec.PrepareInputOutputSpec(flow);
