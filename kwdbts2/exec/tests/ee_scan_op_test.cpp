...
class SpecScan : public SpecBase {
 public:
  explicit SpecScan(k_uint64 table_id) : SpecBase(table_id) {
    agg_push_down_ = false;
  }
...
class TestScanOp : public OperatorTestBase {
 public:
  TestScanOp() : OperatorTestBase() {
  }
...
// Test TableScanOperator functionality
TEST_F(TestScanOp, TestDmlExecScan) {
  TSFlowSpec flow;
  SpecScan scan_spec(table_id_);
  scan_spec.PrepareFlowSpec(flow);
  scan_spec.PrepareInputOutputSpec(flow);
