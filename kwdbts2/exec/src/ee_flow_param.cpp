...
EEIteratorErrCode ReaderPostResolve::ResolveBlockFilter(TSReaderSpec *spec) {
  k_int32 size = spec->blockfilter_size();
  for (k_int32 i = 0; i < size; i++) {
    const TSBlockFilter *filter = &(spec->blockfilter(i));
